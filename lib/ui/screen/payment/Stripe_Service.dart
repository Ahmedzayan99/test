...
class StripeService {
  ...
  static Future<StripeTransactionResponse> payWithPaymentSheet({String? amount, String? currency, String? from, BuildContext? context}) async {
    try {
      ...
      /* final result = await Api.post(body: body, url:'${StripeService.paymentApiUrl}/$stripePayId', token: true, errorCode: false);
      var statusOfTransaction = result['status']; */
  ...
  static Future<Map<String, dynamic>?> createPaymentIntent(String? amount, String? currency, String? from, BuildContext? context) async {
    ...
    try {
      ...
      /* final result = await Api.post(body: parameter, url:StripeService.paymentApiUrl, token: true, errorCode: false);
      return result; */
    } catch (err) {
      print(err.toString());
    }
